...
> [返回项目 README](./README.md) | [English README](./READ_EN.md) | [English version](./summary_EN.md)
...
11. 针对选股能力排名，计算十分组季度/月度多空收益、TOP15 相对 `885001.WI` 的超额收益和累计超额净值。
12. 对选股能力 TOP15、十分组各组、多空组合和 TOP15 超额回测统一计算年化收益、年化波动率、最大回撤和信息比率。
13. 计算选股、择时和综合能力的季度 IC、Rank IC、ICIR、年化 ICIR 和显著性统计。
...
| `fun_RR_plus.ipynb` | 当前函数化主流程：新择时能力、选股能力绩效分析、季度滚动回测、IC/Rank IC |
...
|---|---|---|---|
| `fun_RR_plus.ipynb` | 当前 RR 回归、能力评分、季度滚动回测、选股能力绩效分析和 IC 检验 | 基金日频面板、持仓、宽基/行业/Barra 因子、基金经理、`885001.WI.xlsx` | `outs`、十分组和 TOP15 收益、TOP15 超额收益、季度/月度多空收益、统一绩效指标、IC/Rank IC 汇总与明细 |
...
- `build_decile_cum_return()` 使用实际卖出日串联各期十分组收益；选股能力曲线同时包含第一组减第十组的多空累计净值。
- `build_top15_cum_return()` 串联选股能力 TOP15 的季度收益和累计净值。
- `load_benchmark_885001()` 读取 `基金数据/885001.WI.xlsx` 中的万得偏股混合型基金指数日度收盘价。
- `build_top15_excess_return()` 按实际买卖日计算 TOP15、基准、单期超额收益和累计超额净值。
- `calculate_performance_metrics()` 统一计算几何年化收益、年化波动率、最大回撤和信息比率。
- `build_decile_performance_summary()` 对选股能力十分组第 1 至第 10 组相对 `885001.WI` 计算绩效指标，并对第一组减第十组多空组合使用零收益基准。
...
- `build_stock_selection_long_short_return()` 对选股能力第一组减第十组计算季度胜率、年化收益、年化波动率、最大回撤和信息比率。
- `build_decile_monthly_long_short_win_rate()` 固定季度初选股能力分组名单，按完整自然月计算多空收益和月度胜率。
- notebook 附录“结果调用”已列出绩效汇总、逐期明细、累计曲线和图表对象的变量名。

当前 `outs.pkl` 的 66 期真实数据验证结果：
...
月度多空回测包含 198 个有效月份，其中 122 个月为正，月度胜率为 61.62%，平均月度多空收益为 0.368%。
...
回归暴露 + 持仓暴露 + 基金经理 + 基金公司
    -> 选股/择时/综合能力
    -> TOP15 和十分组下一季度回测
    -> 选股能力 TOP15/十分组统一绩效指标
    -> TOP15 相对 885001.WI 超额收益
    -> 选股能力第一组减第十组的季度/月度多空收益和胜率
```
...
- 选股能力季度多空收益：当前代码第一组为最高分组、第十组为最低分组，定义为 `第一组平均收益 - 第十组平均收益`。
- 多空季度/月度胜率：两组收益均有效的期间中，多空收益严格大于 0 的期间占比；月度计算固定季度初名单，并从下一个完整自然月开始。
- 年化收益：季度累计净值的几何年化，即 `ending_nav ** (4 / periods) - 1`。
- 年化波动率：季度收益样本标准差乘 `sqrt(4)`。
- 最大回撤：累计净值相对历史峰值的最小跌幅，历史峰值包含初始净值 1.0。
- 信息比率：季度主动收益均值除以主动收益样本标准差，再乘 `sqrt(4)`；TOP15 和十分组各组使用 `885001.WI` 为基准，多空组合使用零收益基准。
- TOP15 超额累计净值：TOP15 累计净值除以 `885001.WI` 累计净值。
...
- `NAVReturn.ipynb` 是否已在当前正式 Feather 上执行仍需通过数据生成记录确认；
- 当前 `outs.pkl` 已完成 66 期绩效分析验证，但若上游正式 Feather 刷新，仍需重新生成 `outs`；
...
6. 运行选股能力 TOP15、十分组、TOP15 超额收益和季度/月度多空分析单元，生成统一绩效指标和图表。
...
9. `fun_RR_plus.ipynb`：重新加载数据，执行季度滚动回归与能力回测，并生成选股能力 TOP15、十分组、超额收益、季度/月度多空、统一绩效指标、IC、Rank IC 和 ICIR 结果。
